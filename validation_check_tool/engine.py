...
def _match_line_no(t: str) -> bool:
    ...
    return _lacks(t, r"SERVICE", r"\bSIZE\b", r"CLASS", r"FLUID", r"DIAMETER", r"UOM", r"UNIT", r"DESCRIPTION",
                  r"INSULATION", r"TRACING", r"JACKET")
...
FAMILY_KEYWORDS = {
    "FT": [
        ...
        "TEMPERATURE TX", "TEMP GAUGE WITH WELL", "VENTURI", "ORIFICE",
...
def _line_no_stats(df: pd.DataFrame, col_idx: int, data_start: int,
    ...
    """Counts how many of a column's first data values have an
    identifier-like line-number shape: at least one dash and at least one
    letter (e.g. "301-ATM-0007", or a simpler "TI-0129" tag-style Line No
    used by some projects instead of Baltic's own 3-segment convention).
    Returns (values seen, values matching)."""
    count = matches = 0
    for r in range(data_start, len(df)):
        v = df.iat[r, col_idx]
        if pd.isna(v):
        ...
        if not s:
            continue
        count += 1
        if len(s.split("-")) >= 2 and re.search(r"[A-Za-z]", s):
            matches += 1
        if count >= sample:
            break
...
REPORT_HEADERS = (
    ...
    + ["Source Sheet", "Result", "Checked", "Remark"]
)
...
CHECKED_COL = RESULT_COL + 1
REMARK_COL = CHECKED_COL + 1
CHECKED_MARK = "V"
...
class ReportStats:
    ...
    carried_over_checks: int = 0
    carried_over_remarks: int = 0


def load_previous_checks(report_path: str) -> dict[tuple[str, str], dict[str, str]]:
    """Reads a previously generated Validation Report and returns
    {(Line No, Tag No): {"checked": ..., "remark": ...}} for every row that
    had something in its Checked and/or Remark column, so a new report can
    carry those forward instead of making the user re-review rows (or
    re-type notes) they already went through. Looks headers up by name (not
    a fixed column index) so this keeps working even if an older report was
    generated by a version of the tool with a different column layout."""
    try:
    ...
    if header_row is None:
        return {}
    col_idx = {cell.value: cell.column for cell in header_row if cell.value}
    if "Line No" not in col_idx or "Tag No" not in col_idx:
        return {}
    line_col, tag_col = col_idx["Line No"], col_idx["Tag No"]
    checked_col = col_idx.get("Checked")
    remark_col = col_idx.get("Remark")
    if not checked_col and not remark_col:
        return {}

    checks: dict[tuple[str, str], dict[str, str]] = {}
    last_line_no = ""
    for row in ws.iter_rows(min_row=2):
        line_val = clean(row[line_col - 1].value)
        if line_val:
            last_line_no = line_val
        tag_val = clean(row[tag_col - 1].value)
        checked_val = clean(row[checked_col - 1].value) if checked_col else ""
        remark_val = clean(row[remark_col - 1].value) if remark_col else ""
        if tag_val and (checked_val or remark_val):
            checks[(last_line_no, tag_val)] = {"checked": checked_val, "remark": remark_val}
    return checks


def load_last_report_path(saved_config: dict) -> str | None:
    return saved_config.get("_last_report_path") or None


def save_last_report_path(config_path: str, report_path: str) -> None:
    config = load_config(config_path)
    config["_last_report_path"] = report_path
    Path(config_path).write_text(json.dumps(config, indent=4, ensure_ascii=False), encoding="utf-8")


def build_report(master_lines: list[MasterLine], instrument_rows: list[InstrumentRow],
                  output_path: str, previous_checks: dict[tuple[str, str], str] | None = None) -> ReportStats:
    wb = Workbook()
    ws = wb.active
    ws.title = "Validation"
    ...
    def carry_over(row: int, line_no: str, tag: str) -> None:
        entry = previous_checks.get((line_no, tag))
        if not entry:
            return
        if entry.get("checked"):
            ws.cell(row, CHECKED_COL, entry["checked"])
            stats.carried_over_checks += 1
        if entry.get("remark"):
            ws.cell(row, REMARK_COL, entry["remark"])
            stats.carried_over_remarks += 1
    ...
    for line in master_lines:
        ...
        carry_over(excel_row, line.line_no, "MASTER")
        for c in range(1, n_cols + 1):
            if c in (CHECKED_COL, REMARK_COL):
                continue
        ...
        if not matches:
            ...
            carry_over(excel_row, line.line_no, "No Related Item")
            excel_row += 1
        ...
        for inst, results, final in match_results:
            ...
            carry_over(excel_row, line.line_no, inst.tag)
    ...
    if excel_row > 2:
        ...
        # text, so re-running this tool later can reliably recognize a
        # checked row instead of guessing at whatever mark someone typed.
        checked_range = f"{get_column_letter(CHECKED_COL)}2:{get_column_letter(CHECKED_COL)}{excel_row - 1}"
        dv = DataValidation(type="list", formula1=f'"{CHECKED_MARK}"', allow_blank=True)
        ws.add_data_validation(dv)
        dv.add(checked_range)
