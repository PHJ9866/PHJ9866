...
def load_previous_checks(report_path: str) -> dict[tuple[str, str], str]:
    """Reads a previously generated Validation Report and returns
    {(Line No, Tag No): Checked value} for every row that had something in
    its Checked column, so a new report can carry those marks forward
    instead of making the user re-review rows they already went through.
    Looks headers up by name (not a fixed column index) so this keeps
    working even if an older report was generated by a version of the tool
    ...
    if "Line No" not in col_idx or "Tag No" not in col_idx or "Checked" not in col_idx:
        return {}
    line_col, tag_col, checked_col = col_idx["Line No"], col_idx["Tag No"], col_idx["Checked"]
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        checked_val = clean(row[checked_col - 1].value)
        if tag_val and checked_val:
            checks[(last_line_no, tag_val)] = checked_val
    return checks
...
def build_report(master_lines: list[MasterLine], instrument_rows: list[InstrumentRow],
    ...
    def carry_over(row: int, line_no: str, tag: str) -> None:
        mark = previous_checks.get((line_no, tag))
        if mark:
            ws.cell(row, CHECKED_COL, mark)
